...
describe('reading memory', () => {
    ...
    it('should return a 16 digit number with intcode 1102,34915192,34915192,7,4,7,99,0', () => {
        expect(
            String(
                exec({ intcode: [1102, 34915192, 34915192, 7, 4, 7, 99, 0] })
                    .output
            ).length
        ).toBe(16)
    })
...
describe('opcodes', () => {
    ...
    it('should parse opcode 1 correctly', () => {
        expect(exec({ intcode: [1, 0, 0, 0] }).intcode).toStrictEqual([
            2,
            0,
            0,
            0,
    ...
    it('should parse opcode 2 correctly', () => {
        expect(exec({ intcode: [2, 0, 0, 0] }).intcode).toStrictEqual([
            4,
            0,
            0,
            0,
    ...
    it('should be able to compare against the input with opcode 7 and 8', () => {
        ...
        // input === 8
        expect(
            exec({ intcode: [3, 9, 8, 9, 10, 9, 4, 9, 99, -1, 8], input: 8 })
                .output
        ).toBe(1)
        expect(
            exec({ intcode: [3, 9, 8, 9, 10, 9, 4, 9, 99, -1, 8], input: 5 })
                .output
        ).toBe(0)
        // input < 8
        expect(
            exec({ intcode: [3, 9, 7, 9, 10, 9, 4, 9, 99, -1, 8], input: 13 })
                .output
        ).toBe(0)
        expect(
            exec({ intcode: [3, 9, 7, 9, 10, 9, 4, 9, 99, -1, 8], input: 5 })
                .output
        ).toBe(1)
        // immediate mode
        // input === 8
        ...
        // input < 8
        expect(
            exec({ intcode: [3, 3, 1107, -1, 8, 3, 4, 3, 99], input: 13 })
                .output
        ).toBe(0)
        expect(
